      ...
      {mobileOpen && (
        <div className="md:hidden bg-lms-nav border-t border-border/10 p-4 space-y-3">
          <div className="relative">
            ...
            <Input
              placeholder="Search courses..."
              className="pl-10 bg-foreground/10 border-none text-lms-nav-text placeholder:text-muted-foreground/60"
            />
          </div>
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="block py-2 text-sm font-medium text-lms-nav-text/80 hover:text-primary"
          ...
          <div className="flex gap-2 pt-2">
            <Button variant="outline" size="sm" className="flex-1 border-primary/30 text-lms-nav-text">Log In</Button>
            <Button size="sm" className="flex-1 gradient-primary text-primary-foreground">Sign Up</Button>
          </div>
        </div>
      )}
    </nav>
